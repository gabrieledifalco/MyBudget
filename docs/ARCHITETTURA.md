...
Il repository segue la struttura minima richiesta: `app/`, `agents/`, `presentation/`, `README.md`.

Dentro `app/` la soluzione è un monolite con **frontend e backend fisicamente separati** in due
workspace npm. Nessun file sorgente è condiviso tra i due: chi lavora sul FE tocca solo
`app/frontend/`, chi lavora sul BE solo `app/backend/`. I conflitti git restano quindi circoscritti.

```
MyBudget/
├── package.json              # workspace root: script di avvio combinati
├── README.md                 # specifica funzionale
├── app/                      # la soluzione sviluppata
│   ├── backend/              # API REST Node.js (JavaScript ESM)
│   │   ├── prisma/           # schema dati + migrazioni + seed categorie
│   │   └── src/
...
`http://localhost:4000`; in produzione il backend può servire `app/frontend/dist`
...
npm run db:migrate --workspace @mybudget/backend   # crea lo schema SQLite
npm run db:seed --workspace @mybudget/backend      # categorie predefinite
